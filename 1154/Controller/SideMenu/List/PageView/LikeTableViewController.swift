...
class LikeTableViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, ListTableViewCellDelegate {
    ...
    func loadLikeSubmitList(){
        DispatchQueue.global().async {
            ...
            Firestore.firestore().collection("users").document(uid).collection("like").order(by: "date", descending: true).getDocuments { (snapshot, error) in
                if error != nil{
                }else{
